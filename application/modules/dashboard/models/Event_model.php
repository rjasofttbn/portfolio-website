...
class Event_model extends CI_Model
{
    ...
    public function edit_eventdata($event_id)
    {
        ...
        $this->db->where_in('b.picture_type', 'event');
    ...
    {
        $this->db->select('a.id,b.id as p_id,a.event_id ,a.event_detail_id,a.created_date, a.event_id ,a.name, a.designation, a.company,b.from_id, b.picture');
        $this->db->from('event_detail_tbl a');
        $this->db->join('picture_tbl b', 'b.from_id = a.event_detail_id', 'left');
        $this->db->join('events_tbl c', 'c.event_id = a.event_id', 'left');
        $this->db->where('b.picture_type', 'event_detail');
        $this->db->where('c.event_id', $event_id);
        $query = $this->db->get();
        return $query->result();
    }
    ...
    public function events_list()
    {
        ...
        $this->db->from('events_tbl a');
        $this->db->join('picture_tbl b', 'b.from_id = a.event_id', 'left');
        $this->db->where('b.picture_type', 'event');
